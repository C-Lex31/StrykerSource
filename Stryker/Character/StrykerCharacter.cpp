...
void AStrykerCharacter::I_ClientHUD()
{
	//InitialzeHUD();
}
...
void AStrykerCharacter::CrosshairLogicUpdate_Implementation()
{
		...
		if (MuzzleSocket)
		{
			...
			if (bCrosshairHasObstacle)
			{
				if (LocalPC)
				{
					...
					UWidgetLayoutLibrary::ProjectWorldLocationToWidgetPosition(PC, CrosshairLocation, ObstacleCrosshairLocation, false);
...
void AStrykerCharacter::Tick(float DeltaSeconds)
{
	...
	if (!HasAuthority())
	{
		//if(IsLocallyControlled())
			//CrosshairLogicUpdate();
			//UKismetSystemLibrary::PrintString(GetWorld(), UKismetStringLibrary::Conv_BoolToString(PC->bomba));
			//UKismetSystemLibrary::PrintString(GetWorld(),UKismetSystemLibrary::GetDisplayName(LocalPC  ));
...
{
	bEliminated = true;
	if (IsLocallyControlled() && Crosshair)
	{
